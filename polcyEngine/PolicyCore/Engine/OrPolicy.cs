...
{
    public class OrPolicy : IPolicy
    {
        private readonly IPolicy _first;
        private readonly IPolicy _second;

        public string Definition => $"{_first.Definition} or {_second.Definition}";

        public OrPolicy(IPolicy first, IPolicy second)
        {
            _first = first;
            _second = second;
        }
        ...
        public PolicyResult Check()
        {
            ...
            return PolicyResult.FailedResult($"Neither policy passed:", Definition);
        }
